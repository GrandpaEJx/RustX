...
impl Value {
                    ...
                    if let Some(val) = map.get(method) {
                        match val {
                            ...
                            Value::Function { .. } => {
                                ...
                                Err("Cannot call interpreted function from compiled code (yet)"
                                    .to_string())
                            }
    ...
    pub fn call(&self, args: Vec<Value>) -> Result<Value, String> {
        match self {
            Value::NativeFunction(f) => f(args),
            Value::Function { .. } => {
                Err("Cannot call interpreted function from compiled code".to_string())
            }
