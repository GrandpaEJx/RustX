    ...
    fn analyze_stmt(&mut self, stmt: &Stmt) {
        match stmt {
            ...
            Stmt::Function { params, body, .. } => {
                ...
                for p in params {
                    self.vars.insert(p.clone(), TypeHint::Unknown);
                }
        ...
        match expr {
            ...
            Expr::Binary { left, op, right } => {
                ...
                match op {
                    ...
                    | BinaryOp::Div
                    | BinaryOp::Mod => {
                        if l == TypeHint::Int && r == TypeHint::Int {
                            TypeHint::Int
                        } else {
                            TypeHint::Unknown
                        }
...
impl Transpiler {
    ...
    pub fn transpile(&mut self, stmts: &[Stmt]) -> String {
        ...
        if !used_modules.is_empty() {
            ...
            for module in &used_modules {
                ...
                self.code.push_str(&format!(
                    "static {}: OnceLock<Value> = OnceLock::new();\n",
                    mod_upper
                ));
            }
            self.code.push('\n');
        ...
        self.code
            .push_str("#[allow(unreachable_code)]\nfn main() -> Result<(), String> {\n");
        self.indent_level += 1;
    ...
    fn transpile_stmt_optimized(&mut self, stmt: &Stmt, vars: &HashMap<String, TypeHint>) {
        match stmt {
            ...
            Stmt::While { condition, body } => {
                ...
                if let Expr::Block(stmts) = &**body {
                    for s in stmts {
                        self.transpile_stmt_optimized(s, vars);
                    }
                }
            ...
            Stmt::Function { name, params, body } => {
                self.push_line(&format!(
                    "fn {}(args: Vec<Value>) -> Result<Value, String> {{",
                    name
                ));
                self.indent_level += 1;
                self.enter_scope();
                ...
                self.declare(name.clone());
                self.push_line(&format!(
                    "let {} = Value::NativeFunction(std::sync::Arc::new({}));",
                    name, name
                ));
            }
            Stmt::For {
                ...
                self.push_line(&format!(
                    "for {}_ref in {}.as_array().unwrap().iter() {{",
                    iterator, iter_code
                ));
                self.indent_level += 1;
                self.enter_scope();
    ...
    fn transpile_value_expr(&mut self, expr: &Expr, vars: &HashMap<String, TypeHint>) -> String {
        ...
        match expr {
            ...
            Expr::TemplateString(s) => {
                ...
                if args.is_empty() {
                    ...
                    let args = args
                        .iter()
                        .map(|a| {
                            self.transpile_expr_optimized(&Expr::Ident(a.clone()), vars, false)
                        })
                        .collect::<Vec<_>>()
                        .join(", ");
            ...
            Expr::Array(items) => {
                let items_code: Vec<String> = items
                    .iter()
                    .map(|i| self.transpile_expr_optimized(i, vars, false))
                    .collect();
            ...
            Expr::Map(entries) => {
                ...
                format!(
                    "Value::Map({{ let mut map = HashMap::new(); {} map }})",
                    inserts.join(" ")
                )
            }
            ...
            Expr::MethodCall {
                ...
                let args_code: Vec<String> = args
                    .iter()
                    .map(|a| self.transpile_expr_optimized(a, vars, false))
                    .collect();
            ...
            Expr::Call { callee, args } => {
                ...
                if let Expr::Ident(name) = &**callee {
                    match name.as_str() {
                        "print" => {
                            let prints = args_code
                                .iter()
                                .map(|a| format!("print!(\"{{}} \", {});", a))
                                .collect::<Vec<_>>()
                                .join("\n");
            ...
            Expr::If {
                ...
                if let Some(else_expr) = else_branch {
                    ...
                    format!(
                        "if {}.is_truthy() {{ {}; Value::Null }} else {{ Value::Null }}",
                        cond, then_code
                    )
                }
            }
    ...
    fn transpile_block_body_optimized(
        &mut self,
        stmts: &[Stmt],
        return_last: bool,
        vars: &HashMap<String, TypeHint>,
