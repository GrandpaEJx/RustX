...
impl Interpreter {
    ...
    pub(super) fn builtin_range(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
        let (start, end, step) = match args.len() {
            1 => {
                let end = self.eval_expr(args[0].clone())?.as_int().map_err(RuntimeError::from)?;
                (0, end, 1)
            }
            2 => {
                let start = self.eval_expr(args[0].clone())?.as_int().map_err(RuntimeError::from)?;
                let end = self.eval_expr(args[1].clone())?.as_int().map_err(RuntimeError::from)?;
                (start, end, 1)
            }
            3 => {
                let start = self.eval_expr(args[0].clone())?.as_int().map_err(RuntimeError::from)?;
                let end = self.eval_expr(args[1].clone())?.as_int().map_err(RuntimeError::from)?;
                let step = self.eval_expr(args[2].clone())?.as_int().map_err(RuntimeError::from)?;
                if step == 0 {
                    return Err(RuntimeError::ArgumentError("range step cannot be zero".to_string()));
                }
                (start, end, step)
            }
            _ => return Err(RuntimeError::ArgumentError("range() takes 1, 2, or 3 arguments".to_string())),
        };
    ...
    pub(super) fn builtin_len(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
        if args.len() != 1 {
            return Err(RuntimeError::ArgumentError("len() takes exactly 1 argument".to_string()));
        }
    ...
    pub(super) fn builtin_type(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
        if args.len() != 1 {
            return Err(RuntimeError::ArgumentError("type() takes exactly 1 argument".to_string()));
        }
        let val = self.eval_expr(args[0].clone())?;
    ...
    pub(super) fn builtin_push(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
        if args.len() != 2 {
            return Err(RuntimeError::ArgumentError("push() takes exactly 2 arguments: array and value".to_string()));
        }
        ...
        let array_name = if let Expr::Ident(name) = &args[0] {
            ...
            return Err(RuntimeError::ArgumentError("push() first argument must be an array variable".to_string()));
        };
    ...
    pub(super) fn builtin_pop(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
        if args.len() != 1 {
            return Err(RuntimeError::ArgumentError("pop() takes exactly 1 argument: array".to_string()));
        }
        ...
        let array_name = if let Expr::Ident(name) = &args[0] {
            ...
            return Err(RuntimeError::ArgumentError("pop() argument must be an array variable".to_string()));
        };
    ...
    pub(super) fn builtin_split(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
        if args.len() != 2 { return Err(RuntimeError::ArgumentError("split() takes 2 arguments".to_string())); }
    ...
    pub(super) fn builtin_join(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
        ...
        if args.len() != 2 { return Err(RuntimeError::ArgumentError("join() takes 2 arguments".to_string())); }
        ...
        match (arr, d) {
            ...
            (Value::Array(_), _) => Err(RuntimeError::TypeMismatch { expected: "String".to_string(), found: "other".to_string() }),
            (_, _) => Err(RuntimeError::TypeMismatch { expected: "Array".to_string(), found: "other".to_string() }),
        }
    }

    pub(super) fn builtin_trim(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
        if args.len() != 1 { return Err(RuntimeError::ArgumentError("trim() takes 1 argument".to_string())); }
        self.eval_expr(args[0].clone())?.trim().map_err(RuntimeError::from)
    }

    pub(super) fn builtin_upper(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
        if args.len() != 1 { return Err(RuntimeError::ArgumentError("upper() takes 1 argument".to_string())); }
        self.eval_expr(args[0].clone())?.upper().map_err(RuntimeError::from)
    }

    pub(super) fn builtin_lower(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
         if args.len() != 1 { return Err(RuntimeError::ArgumentError("lower() takes 1 argument".to_string())); }
         self.eval_expr(args[0].clone())?.lower().map_err(RuntimeError::from)
    }

    // === MATH FUNCTIONS ===

    pub(super) fn builtin_abs(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
        if args.len() != 1 { return Err(RuntimeError::ArgumentError("abs() takes 1 argument".to_string())); }
        self.eval_expr(args[0].clone())?.abs().map_err(RuntimeError::from)
    }

    pub(super) fn builtin_min(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
        if args.len() != 2 { return Err(RuntimeError::ArgumentError("min() takes 2 arguments".to_string())); }
    ...
    pub(super) fn builtin_max(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
        if args.len() != 2 { return Err(RuntimeError::ArgumentError("max() takes 2 arguments".to_string())); }
        let a = self.eval_expr(args[0].clone())?;
        let b = self.eval_expr(args[1].clone())?;
        match (a, b) {
    ...
    pub(super) fn builtin_floor(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
         if args.len() != 1 { return Err(RuntimeError::ArgumentError("floor() takes 1 argument".to_string())); }
         self.eval_expr(args[0].clone())?.floor().map_err(RuntimeError::from)
    }

    pub(super) fn builtin_ceil(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
         if args.len() != 1 { return Err(RuntimeError::ArgumentError("ceil() takes 1 argument".to_string())); }
         self.eval_expr(args[0].clone())?.ceil().map_err(RuntimeError::from)
    }

    pub(super) fn builtin_round(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
         if args.len() != 1 { return Err(RuntimeError::ArgumentError("round() takes 1 argument".to_string())); }
         self.eval_expr(args[0].clone())?.round().map_err(RuntimeError::from)
    }
    ...
    pub(super) fn builtin_map(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
        if args.len() != 2 {
            return Err(RuntimeError::ArgumentError("map() takes exactly 2 arguments: array and callback".to_string()));
        }
        let arr = self.eval_expr(args[0].clone())?;
    ...
    pub(super) fn builtin_filter(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
        if args.len() != 2 {
            return Err(RuntimeError::ArgumentError("filter() takes exactly 2 arguments: array and callback".to_string()));
        }
        let arr = self.eval_expr(args[0].clone())?;
    ...
    pub(super) fn builtin_reduce(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
        if args.len() < 2 || args.len() > 3 {
             return Err(RuntimeError::ArgumentError("reduce() takes 2 or 3 arguments".to_string()));
        }
        let arr = self.eval_expr(args[0].clone())?;
    ...
    pub(super) fn builtin_reverse(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
        if args.len() != 1 { return Err(RuntimeError::ArgumentError("reverse() takes 1 argument".to_string())); }
    ...
    pub(super) fn builtin_sort(&mut self, args: Vec<Expr>) -> InterpreterResult<Value> {
        if args.len() != 1 { return Err(RuntimeError::ArgumentError("sort() takes 1 argument".to_string())); }
        let mut arr = self.eval_expr(args[0].clone())?;
    ...
    pub(super) fn logic_map(&mut self, arr: Value, callback: Expr) -> InterpreterResult<Value> {
         let items = match arr {
            Value::Array(a) => a,
            _ => return Err(RuntimeError::TypeMismatch { expected: "Array".to_string(), found: "other".to_string() }),
        };
    ...
    pub(super) fn logic_reduce(&mut self, arr: Value, callback: Expr, initial: Option<Expr>) -> InterpreterResult<Value> {
        let items = match arr {
            Value::Array(a) => a,
            _ => return Err(RuntimeError::TypeMismatch { expected: "Array".to_string(), found: "other".to_string() }),
        };
        ...
        let mut accumulator = if let Some(init_expr) = initial {
            ...
            iterator.next().ok_or_else(|| RuntimeError::from("reduce() on empty array with no initial value"))?
        };
